...
namespace Holdem
{
    ...
    public class Table
    {
        ...
        public Table(PlayerList players)
            : this(players, 500, 1000)
        {
        }

        public Table(PlayerList players, int smallBlindAmount, int bigBlindAmount)
        {
            ...
            roundCounter = 0;
            turnCount = 0;
            dealerPosition = players.Count > 0 ? rand.Next(players.Count) : 0;
            //set blind amount and position
            smallBlind.Amount = smallBlindAmount;
            bigBlind.Amount = bigBlindAmount;
            mainPot.SmallBlind = smallBlindAmount;
            mainPot.BigBlind = bigBlindAmount;
            smallBlind.position = dealerPosition + 1;
            bigBlind.position = dealerPosition + 2;
            currentIndex = dealerPosition;
        }
        public Table()
            : this(new PlayerList(), 500, 1000)
        {
        }
        //indexer of players
        ...
        {
            if (players.Count == 0)
                throw new InvalidOperationException("Cannot increment turn order without players.");
            ...
            while (players.GetPlayer(ref currentIndex).IsFolded()||players.GetPlayer(ref currentIndex).isbusted||players.GetPlayer(ref currentIndex).ChipStack==0)
            {
                ...
                if (attempts > players.Count)
                    throw new InvalidOperationException("No active player is available.");
            }
        ...
        public int incrementIndexShowdown(int currentIndex)
        {
            ...
            while (players.GetPlayer(ref currentIndex).IsFolded() || players.GetPlayer(ref currentIndex).isbusted)
            {
                ...
                if (attempts > players.Count)
                    throw new InvalidOperationException("No showdown player is available.");
            }
            players.GetPlayer(ref currentIndex);
            return currentIndex;
        }
        ...
        {
            if (players.Count == 0)
                throw new InvalidOperationException("Cannot decrement turn order without players.");
        ...
        {
            if (players.Count == 0)
                throw new InvalidOperationException("Cannot pay blinds without players.");
        ...
        {
            if (mainPot.getPlayersInPot().Count == 0)
            {
                winnermessage = string.Empty;
                return;
            }
        ...
        {
            if (mainPot.getPlayersInPot().Count < 2)
                return false;
